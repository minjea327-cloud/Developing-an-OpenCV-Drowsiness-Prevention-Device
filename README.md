...
본 실험은 OpenCV와 라즈베리 파이를 활용하여 운전자 및 사용자의 사고를 예방하는 **'실시간 졸음 감지 및 경고 시스템'** 구현을 목적으로 한다.

카메라로 입력된 영상은 연산 속도를 높이기 위해 흑백으로 변환되며, 명암 차이를 분석하는 **Haar Cascade 알고리즘**을 통해 사용자의 얼굴과 눈 영역을 차례로 검출한다.

시스템은 실시간으로 감지된 눈의 개수가 1개 이하로 떨어지는 상황(눈을 감음)을 졸음 상태로 판별한다. 위험이 감지되면 GPIO 핀으로 연결된 피에조 부저를 즉각 작동시켜 경고음을 울림으로써, 사용자의 각성을 유도하고 안전사고를 미연에 방지하는 저비용·고효율의 스마트 안전 디바이스를 구축한다.
## 실험장비
1. 라즈베리파이
2. 브레드보드
3. 능동부저
